...
st.title('Querying on MySQL database and showing output in streamlit')


#query1
with engine.connect() as connection:
    result_1 = connection.execute(text("""select District,sum(Population) as Total_Population 
                                        from census 
                                        group by District 
                                        order by District"""))
    output_1 = result1.fetchall()
...
#query2
with engine.connect() as connection:
    result2 = connection.execute(text("""SELECT 
                                            District,
...
with engine.connect() as connection:
    result11 = connection.execute(text("select `State_UT`,sum(coalesce(Households_Rural,0))+sum(coalesce(Urban_Households,0)) as Households from census group by `State_UT` order by `State_UT`"))
    output11 = result11.fetchall()
df11 = pd.DataFrame(output11)
st.markdown(" 11.What is the total number of households in each state?   \n **Query:**  \n select `State_UT`,sum(coalesce(Households_Rural,0))+sum(coalesce(Urban_Households,0)) as Households from census group by `State_UT` order by `State_UT`  \n**Output:** ")
st.dataframe(df11)

with engine.connect() as connection:
    result12 = connection.execute(text("select `State_UT`,sum(Having_latrine_facility_within_the_premises_Total_Households) as Households_with_latrine_facility_within_the_premises from census group by `State_UT` order by `State_UT`"))
    output12 = result12.fetchall()
df12 = pd.DataFrame(output12)
st.markdown(" 12.How many households have a latrine facility within the premises in each state?   \n **Query:**  \n select `State_UT`,sum(Having_latrine_facility_within_the_premises_Total_Households) as Households_with_latrine_facility_within_the_premises from census group by `State_UT` order by `State_UT`  \n**Output:** ")
st.dataframe(df12)

with engine.connect() as connection:
    result13 = connection.execute(text("select `State_UT`,round(AVG(coalesce(Household_size_1_to_2_persons,0))) as Household_size_1_to_2_persons,round(AVG(coalesce(Household_size_3_persons_Households,0))) as Household_size_3_persons_Households,round(AVG(coalesce(Household_size_4_persons_Households,0))) as Household_size_4_persons_Households,round(AVG(coalesce(Household_size_5_persons_Households,0))) as Household_size_5_persons_Households,round(AVG(coalesce(Household_size_6_8_persons_Households,0))) as Household_size_6_8_persons_Households,round(AVG(coalesce(Household_size_9_persons_and_above_Households,0))) as Household_size_9_persons_and_above_Households from census group by `State_UT` order by `State_UT`"))
...
with engine.connect() as connection:
    result16 = connection.execute(text("select `State_UT`,sum(Location_of_drinking_water_source_Near_the_premises_Households) as Location_of_drinking_water_source_Near_the_premises_Households from census group by `State_UT` order by `State_UT`"))
    output16 = result16.fetchall()
df16 = pd.DataFrame(output16)
st.markdown(" 16.How many households have access to drinking water sources near the premises in each state?   \n **Query:**  \n select `State_UT`,sum(Location_of_drinking_water_source_Near_the_premises_Households) as Location_of_drinking_water_source_Near_the_premises_Households from census group by `State_UT` order by `State_UT`  \n**Output:** ")
st.dataframe(df16) 

with engine.connect() as connection:
    result17 = connection.execute(text("select `State_UT`,round(AVG(coalesce(Power_Parity_Less_than_Rs_45000,0))) as Power_Parity_Less_than_Rs_45000,round(AVG(coalesce(Power_Parity_Rs_45000_150000,0))) as Power_Parity_Rs_45000_150000,round(AVG(coalesce(Power_Parity_Rs_150000_330000,0))) as Power_Parity_Rs_150000_330000,round(AVG(coalesce(Power_Parity_Rs_330000_545000,0))) as Power_Parity_Rs_330000_545000,round(AVG(coalesce(Power_Parity_Above_Rs_545000,0))) as Power_Parity_Above_Rs_545000 from census group by `State/UT` order by `State_UT`"))
...
with engine.connect() as connection:
    result19 = connection.execute(text("select `State_UT`,sum(coalesce(Power_Parity_Less_than_Rs_45000,0)) as Power_Parity_Less_than_Rs_45000,sum(coalesce(Power_Parity_Rs_45000_150000,0)) as Power_Parity_Rs_45000_150000,sum(coalesce(Power_Parity_Rs_150000_240000,0)) as Power_Parity_Rs_150000_240000 from census group by `State_UT` order by `State_UT`"))
    output19 = result19.fetchall()
df19 = pd.DataFrame(output19)
st.markdown(" 19.How many households fall below the poverty line in each state based on the power parity categories?   \n **Query:**  \n select `State_UT`,sum(coalesce(Power_Parity_Less_than_Rs_45000,0)) as Power_Parity_Less_than_Rs_45000,sum(coalesce(Power_Parity_Rs_45000_150000,0)) as Power_Parity_Rs_45000_150000,sum(coalesce(Power_Parity_Rs_150000_240000,0)) as Power_Parity_Rs_150000_240000 from census group by `State_UT` order by `State_UT`  \n**Output:** ")
st.dataframe(df19) 

with engine.connect() as connection:
    result20 = connection.execute(text("select `State_UT` , round((sum(Literate)/sum(Population))*100) as  Literacy_Rate from census group by `State_UT` order by `State_UT`"))
    output20 = result20.fetchall()
df20 = pd.DataFrame(output20)
st.markdown(" 20.What is the overall literacy rate (percentage of literate population) in each state?   \n **Query:**  \n select `State_UT` , round((sum(Literate)/sum(Population))*100) as  Literacy_Rate from census group by `State_UT` order by `State_UT`  \n**Output:** ")
st.dataframe(df20) 